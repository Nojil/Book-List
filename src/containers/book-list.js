    ...
    renderList() {
        return this.props.books.map((book) => {
            return (
                <li
                    key={book.title}
                    onClick={() => this.props.selectBook(book)} className="list-group-item">{book.title}</li>
            );
        });
    }
    render() {
...
//connect take a function and a component and produces a container, which is aware of the state contained by redux
